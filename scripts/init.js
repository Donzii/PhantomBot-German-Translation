...
(function() {
        ...
        if (!isModuleLoaded(scriptName) || force) {
            if (scriptName.endsWith('.js')) {
                try {
                    ...
                    consoleLn('Laden von Modul "' + scriptName + '" fehlgeschlagen: ' + ex);
                }
            }
        }
    }
    ...
    function addHook(hookName, handler) {
        ...
        if (hookName !== 'initReady' && $api.exists(hookName) == false) {
            Packages.com.gmt2001.Console.err.printlnRhino('[addHook()@init.js:254] Haken "' + hookName + '" konnte nicht registriert werden, da es kein solches Ereignis gibt.');
        } else if (i !== -1) {
    ...
    function init() {
        ...
        if ($.isNightly) {
            ...
            consoleLn('Bitte gib bei der Meldung von Fehlern oder Problemen an, dass es sich um eine Vorabversion handelt.');
        } else {
            consoleLn('Für Unterstützung besuche bitte: https://community.phantombot.tv');
        }
        consoleLn('');
    }
